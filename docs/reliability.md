...
| INV-06 | Repeated malformed model output produces a fallback requiring human review.           | Enforced; persistence planned                            | `src/domain/decision.schemas.ts`; `src/services/reliable-analysis.service.ts`         | `tests/unit/reliable-analysis.service.test.ts` covers invalid-then-valid and invalid-then-invalid                               |
| INV-07 | Provider failure produces human review or an explicit failed/fallback analysis state. | Enforced; persistence/API integration planned            | `src/llm/llm-errors.ts`; `src/services/reliable-analysis.service.ts`                  | `tests/unit/reliable-analysis.service.test.ts` covers retryable, exhausted, and permanent provider failures                     |
